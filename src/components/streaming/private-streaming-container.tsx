...
class PrivateStreamingContainer extends PureComponent<IProps, IState> {
  ...
  async handelWebRTCAdaptorCallback(info: WEBRTC_ADAPTOR_INFORMATIONS, obj: any) {
    ...
    const {
      settings, webRTCAdaptor, configs, isRoomOwner = false
    } = this.props;
    this.socket = this.context;
  ...
  initSocketEvent() {
    ...
    this.socket.on(STREAM_LEAVED, (data: { streamId: string; conversationId: string }) => {
      const { conversationId, streamId } = this.state;
      if (!conversationId || conversationId !== data.conversationId || streamId === data.streamId) {
        return;
      }
  ...
  leave() {
    ...
    if (process.browser) {
      ...
      this.leaveStream();
      if (isRoomOwner) { window.location.reload(); } else {
        setTimeout(() => {
          window.location.href = '/';
        }, 5 * 1000);
        message.success('Stream has been stopped. You will be redirected to homepage in 5 seconds');
      }
    }
  }
  ...
  render() {
    ...
    return (
      <>
        <div className={classnames('private-streaming-container', containerClassName)} hidden={!publish_started}>
          <video
        ...
        <div>
          {!initialized ? (
            <Button type="primary" onClick={onClick} loading={loading} disabled={loading} block>
              Start Streaming
            </Button>
          ) : (
            <Button type="primary" onClick={this.leave.bind(this)} block disabled={loading}>
              Stop Streaming
            </Button>
          )}
        </div>
